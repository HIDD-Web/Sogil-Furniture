...
export default function GuestTrackOrder() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!f.order_number || !f.phone) return toast.error("Isi nomor pesanan & No HP");
    setLoading(true);
    try {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12 sm:px-6">
      <div className="rounded-2xl border border-[#E5DCC5] bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-3 text-center font-heading text-xl font-bold text-[#2C1E16]">Lacak Pesanan</h1>
        <p className="mt-1 text-center text-xs text-[#8B7355]">Masukkan nomor pesanan & No HP yang dipakai saat memesan. Tidak perlu akun.</p>
        <form onSubmit={submit} className="mt-5 space-y-3">
          <div><Label className="mb-1 block text-sm">Nomor Pesanan</Label><Input value={f.order_number} onChange={(e) => setF({ ...f, order_number: e.target.value })} placeholder="SGF-20260827-001" data-testid="track-order-number" className="bg-white" required /></div>
          <div><Label className="mb-1 block text-sm">No HP (+kode negara)</Label><Input value={f.phone} onChange={(e) => setF({ ...f, phone: e.target.value })} placeholder="+201234567890" data-testid="track-phone" className="bg-white" required /></div>
          <Button type="submit" disabled={loading} data-testid="track-submit" className="h-11 w-full rounded-xl bg-[#8B5A2B] hover:bg-[#6B4423]">{loading ? "Mencari..." : "Lacak"}</Button>
        </form>
      </div>
      ...
      {order && (
        <div className="mt-6 rounded-2xl border border-[#E5DCC5] bg-white p-6 shadow-sm" data-testid="track-result">
          ...
          <div className="mt-1 text-sm text-[#8B7355]">Atas nama {order.customer_name} · {(order.created_at || "").slice(0, 10)}</div>
